...
function search() {
	  ...
	  $("#searchForm").attr("action", "qs/costLedger/list");
	  $("#searchForm").submit();
	   ...
	   <div>
			<table width="100%" class="search">
				<tr>
					...
					<td>责任人：</td>
			    	<td><form:input path="dto.respPersonName"  onfocus="userAutoComplete()"/></td>
					<td>添加时间：</td>
					<td colspan="2">
				...
				<tr>
					...
					<td>处理人：</td>
					<td><form:input path="dto.dealPersonName"  onfocus="userAutoComplete()"/></td>
					<td>责任岗位：</td>
					<td>
						<form:input path="dto.jobName" onfocus="jobAutoComplete()"/>　
			        </td>
			        	<td></td>
			        <td>
	    	...
	    	<tr>
	   		...
	   		<c:forEach items="${dto.dataList}" var="qualityCost">
	   		 <tr>
		   		 ...
		   		 <td class="jobName">${qualityCost.jobName }</td>
		   		 <td class="respPersonName">${qualityCost.respPersonName }</td>
