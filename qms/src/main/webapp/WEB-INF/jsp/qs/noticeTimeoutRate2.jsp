...
require(
    ...
    function (ec) {
    	...
    	var noticeTimeoutNums = new Array();
    	var totalNums = new Array();
    	var rates = new Array();
		<c:forEach items="${dataList}" var="map">
			depNames.push('${map.statisticDate}');
			noticeTimeoutNums.push('${map.noticeTimeOutOrdNumTotal}');
			totalNums.push('${map.signedOrdNumTotal}');
			rates.push('${map.rate}');
		</c:forEach>
    
        // 基于准备好的dom，初始化echarts图表
        var ntoChart = ec.init(document.getElementById('main'));
        option = {
		    tooltip : {
		        trigger: 'axis',
		        formatter: "{b}<br/>" + 
...
function search() {
	$("#ntoForm").attr("action", "qs/substdOrderSnapshot/showNoticeTimeoutRate2");
	$("#ntoForm").submit();
}
</script>
</head>
<body>
<form id="ntoForm" method="post" action="qs/substdOrderSnapshot/showNoticeTimeoutRate2">
<input type="hidden" name="tokenKey" value="${tokenKey}">
<input type="hidden" name="tokenValue" value="${tokenValue}">
<div id="accordion">
<h3>出团通知超时率-统计日期维度</h3>
<div align="right">
<table width="70%">
	<tr>
		<td>
