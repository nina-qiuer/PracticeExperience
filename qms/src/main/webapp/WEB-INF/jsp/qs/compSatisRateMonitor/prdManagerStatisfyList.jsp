...
function search() {
	...
	$("#dataForm").attr("action", "qs/compSatisRate/prdManagerList/"+targetId);
	$("#dataForm").submit();
}

function addSatisfy(prdManager,targetId) {
   ...
   if(targetId ==0 ){
   	 ...
   	 layer.alert("没有配置目标值", {icon: 2});
		 ...
		 var url ="qs/compSatisRate/"+ "prdSpecialistList" + "/" + prdManager + "/" + targetId +"/" +timeType+"/" + nextSearch;
			var htmlStr = '<iframe src="' + url + '" width="100%" height="100%" frameborder="0"></iframe>';
			parent.tabpanel.addTab({
			    title: prdManager+"列表",
			    html: htmlStr
			 });
		 
		 /* parent.addSatisfyTab(prdManage,targetId,"prdSpecialistList",timeType,nextSearch); */
	 }
}
...
<tr>
		...
		<th style="display: none">产品组ID</th>
	...
	<tr>
		...
		<td >
		  <a href="javascript:void(0)" onclick="addSatisfy( '${satisfy.prdManager}', '${satisfy.targetId}')">详情</a>
		</td>
	</tr>
	</c:forEach>
</table>
