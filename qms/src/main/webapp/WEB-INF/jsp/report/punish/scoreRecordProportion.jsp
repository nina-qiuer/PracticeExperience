...
require(
    ...
    function (ec) {
    	if(viewFlag ==1){
    		var qcType = "${dto.qcType}";
	      
	        if(qcType==1){
	        	 qcTypeName = "质检类型一级占比";
	        	 qcTypeNameRank ="质检类型一级记分总数";
	        }else if(qcType==2){
	        	 qcTypeName = "质检类型二级占比";
	        	 qcTypeNameRank ="质检类型二级记分总数";
	        }else if(qcType==3){
	        	 qcTypeName = "质检类型三级占比";
	        	 qcTypeNameRank ="质检类型三级记分总数";
	        }
    	}else if(viewFlag==2) {
	        ...
	        if(depType==1){
	        	qcTypeName = "一级责任部门占比";
	        	qcTypeNameRank ="一级责任部门记分总数";
	        }else if(depType==2){
	        	qcTypeNameRank ="二级责任部门记分总数";
	        }else if(depType==3){
	        	qcTypeName = "三级责任部门占比";
	        	qcTypeNameRank ="三级责任部门记分总数";
	        }
    	}
		else if(viewFlag==3) {
			qcTypeName="关联岗位占比";	
			qcTypeNameRank ="关联岗位记分总数";
		 }
		else if(viewFlag==4) {
			qcTypeName="被处罚人占比";	
			qcTypeNameRank ="被处罚人记分总数";
		}
		
   		var qcTypeNum = new Array();
   		var qcTypeNumRank = new Array();
       	var nums = new Array();
       	var signDeptList = new Array();
       	<c:forEach items="${dataList}" var="map">
       		var tempMap = {};
   			qcTypeNum.push(lineFeed('${map.qcTypeName}'));
   			var rank = parseInt('${map.rank}');
   			if(rank ==0){
   				
   				qcTypeNumRank.push(lineFeed('${map.qcTypeName}'));
   				
   			}else{
   				
   				qcTypeNumRank.push(lineFeed('${map.qcTypeName}') +"["+ rank+"]");
   			}
   			nums.push('${map.num}');
   			tempMap['name'] = lineFeed('${map.qcTypeName}');
...
function search() {
	...
	$("#dataForm").attr("action", "report/QcPunishReportReport/scoreRecordProportion");
	$("#dataForm").submit();
}
...
<table width="100%" class="search">
				<tr>
				   ...
				   <td>
					...
					<td colspan="4">
						<div id="radioset">
							<form:radiobutton path="dto.viewFlag" value="1" label="质检类型" />
							<form:radiobutton path="dto.viewFlag" value="2" label="关联部门" />
							<form:radiobutton path="dto.viewFlag" value="3" label="关联岗位" />
							<form:radiobutton path="dto.viewFlag" value="4" label="被处罚人" />
						</div>
				</td>
				</tr>
				<tr>
