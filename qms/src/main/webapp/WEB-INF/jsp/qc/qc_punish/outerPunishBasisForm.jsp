...
function savePunish(){
	...
	if(val==null||val==''){
		layer.alert("请至少选择一个执行标准！", {
			icon : 2
		});
		return false;
	}
	$.ajax( {
			url : 'qc/outerPunishBasis/addPunishBasis',
...
function deleteBasis(id){
	...
	$.ajax( {
			url : 'qc/outerPunishBasis/deleteBasis',
			data : 	{id:id} ,
			type : 'post',
			dataType:'json',
			cache : false,
...
<input type="hidden" name="opbId"  id="opbId" value="${punishId}">
 <table class="listtable">
		...
		<th width="70">记分处罚</th>
		<th>
		<input type="button"   name ="selectBtn" id="selectBtn" value="添加" class="blue" 
			 onclick="openWinow('添加处罚依据', 'qc/punishStandard/getPunishStandard?punishId=${punishId}&punishObj=2&useFlag=${useFlag}', 700, 400)">
	</th>
		</tr>
		...
		<tr align="center">
			...
			<td>${v.punishStandard.economicPunish }</td>
