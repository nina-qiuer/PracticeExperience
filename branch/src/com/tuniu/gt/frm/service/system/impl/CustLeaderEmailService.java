...
@Service
public class CustLeaderEmailService implements IEmailService {
	...
	public List<String> getReceiverEmails(ComplaintEntity complaint) {
		...
		if (custSaler != null && StringUtil.isNotEmpty(custSaler.getRealName())) {
			...
			if (custSalerManager != null && StringUtil.isNotEmpty(custSalerManager.getRealName())) {
				UserEntity custSalerLeader = ComplaintRestClient.getReporter(custSalerManager.getRealName());
				if (custSalerLeader != null && StringUtil.isNotEmpty(custSalerLeader.getEmail())) {
					emails.add(custSalerLeader.getEmail());
				}
			}
		}
		return emails;
