...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr valign="top">
    	...
    	<table class="listtable" width="100%" border="0" cellspacing="0" cellpadding="0">
    		<c:forEach items="${request.dataList}" var="v"  varStatus="st"> 
    			<c:if test="${st.index%5==0}">
		        	...
		        	<th>
		        		<a href="${manageUrl}-superAdmin?role_id=${v.id}">${v.roleName}</a> 
		        	</th>
    ...
    <td width="60%"><div id="main">
    	<c:if test="${role_id>0}">
    	<form id="role_privilege_form" name="role_privilege_form" method="post" action="${CONFIG.app_url}frm/ajax/privilege/role_privilege-doSuperAdminAllocate">
	        ...
	        <ul id="navigation" class="treeview">
						...
						<c:forEach items="${menuList}" var="v">
							...
							<li>
								<div style="display:none;" id='tree_node_${v.fatherId}_${v.id}'> 
									...
									${ui:makeCheckbox(v.privileges,checkbox_name,rolePrivilegeMap[menu_id].privilegeIds,"id,privilegeName,1")}  
				...
				<li><input type="submit" value="提交" onclick="ajax_do_super_admin_allocate_role_privilege();" ></li> 
			</ul>
		</form>
		</c:if>  
      </td>
