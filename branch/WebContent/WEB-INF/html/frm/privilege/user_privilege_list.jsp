    	...
    	<table class="listtable" width="100%" border="0" cellspacing="0" cellpadding="0">
    		<c:forEach items="${userList}" var="v"  varStatus="st"> 
    			<c:if test="${st.index%5==0}">
		        	...
		        	<th>
		        		<a href="${manageUrl}?user_id=${v.id}">${v.realName}</a>  
		        	</th>
						...
						<c:forEach items="${menuList}" var="v">
							...
							<li>
								<div style="display:none;" id='tree_node_${v.fatherId}_${v.id}'> 
									...
									<input type="checkbox" name="menu_id" value="${v.id}" <c:if test="${rolePrivilegeMap[menu_id] != null}">checked disabled="true"</c:if>> 
