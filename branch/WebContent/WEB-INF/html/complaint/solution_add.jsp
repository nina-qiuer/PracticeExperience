...
$(document).ready(function() {
    ...
    if(cseEntity == null || cseEntity == ""){
    	$("#emailType").val("1");
    	$("#emailName").val('${complaint.orderId}'+"投诉处理进展报备");
    	$("#route").val("${complaint.route}");
    	$("#orderId").val('${complaint.orderId}');
    	$("#agencyName").val('');
    ...
    $('#emailType').change(function(){ 
		...
		if(cseEntity == null || cseEntity == ""){
	    	...
	    	if(val == 1){
	    		...
	    		$("#emailName").val('${complaint.orderId}'+"XXX关于客人XXX受伤情况处理报备");
	    		...
	    		$("#emailName").val('${complaint.orderId}'+"XXX 关于XXX问题处理一级报备");
...
<div class="common-box">
<form name="form" id="form" method="post" enctype="multipart/form-data" action="complaint_solution-addSolution">
	...
	<input type="hidden" name="entity.cardUniqueId" id="cardUniqueId" value="${cardUniqueId }">
	<table class="datatable" width="100%">
		<tr>
			<th width="100" align="right" colspan="2">对客赔偿总额：</th>
			<td><span id="payTotal">0</span> 元 <font color="red">（赔偿总额 = 现金 + 旅游券）</td>
		</tr>
		<tr>
			<th width="100" align="right" colspan="2">联系人：</th>
			<td>
				<input name="entity.userName" id="userName" type="text" size="20" value="${contact.name }">
			</td>
		</tr>
		<tr>
			<th width="100" align="right" colspan="2">联系手机：</th>
			<td>
				<input name="entity.phone" id="phone" type="text" size="20" value="${contact.tel }">
			</td>
		</tr>
		<tr>
			<th id="th_cash" rowspan="7">赔<br>偿<br>现<br>金</th>
			<th width="100" align="right">金额：</th>
			<td><input name="entity.cash" type="text" id="cash" size="20" onblur="computePayTotal()"> 元</td>
		</tr>
		<tr>
			<th width="100" align="right">退款方式：</th>
		    <td>
		    <label><input type="radio" name="entity.payment" id="payment" value="1" onclick="changePaymentItem(1)"> 现金</label>
		    <label><input type="radio" name="entity.payment" id="payment" value="2" onclick="changePaymentItem(2)" checked="checked"> 对私汇款</label>
		    <label><input type="radio" name="entity.payment" id="payment" value="3" onclick="changePaymentItem(3)"> 对公汇款</label>
		    <label><input type="radio" name="entity.payment" id="payment" value="25" onclick="changePaymentItem(25)"> 转单</label>
		    </td>
		</tr>
		<tr id="tr_receiver" style="display: none;">
			<th width="100" align="right">收款人：</th>
		    <td><input name="entity.receiver" type="text" id="receiver" size="20"></td>
		</tr>
		<tr id="tr_idCardNo" style="display: none;">
			<th width="100" align="right">身份证号码：</th>
		    <td><input name="entity.idCardNo" type="text" id="idCardNo" size="20"></td>
		</tr>
		<tr id="tr_collectionUnit">
			<th width="100" align="right">收款人：</th>
		    <td><input type="text" id="collectionUnit" size="40" class="disabled-input" readonly="readonly" onfocus="this.blur()"/></td>
		</tr>
		
		<tr id="tr_big_bank">
			<th width="100" align="right">银行名称：</th>
		    <td><input type="text" id="bigBank" size="40" class="disabled-input" readonly="readonly" onfocus="this.blur()"/>
		    <input type="button" class="blue"  value="填写账户信息" onclick="addAccountWindow()"></td>
		</tr>
		
		<tr id="tr_bank_district">
			<th width="100" align="right">开户行省份城市</th>
			<td>
					<input type="text" size="10" id="bankProvince" class="disabled-input" readonly="readonly" onfocus="this.blur()"/>省
					<input type="text" size="10" id="bankCity" class="disabled-input" readonly="readonly" onfocus="this.blur()"/>市
			</td>
		</tr>
		<tr id="tr_bank">
		    <th width="100" align="right">分行名称：</th>
            <td>
            <input type="text" id="bank" size="40" class="disabled-input" readonly="readonly" onfocus="this.blur()"/>
            </td>
		</tr>
		<tr id="tr_account">
		    <th width="100" align="right">账号：</th>
            <td><input type="text" id="account" size="30" class="disabled-input" readonly="readonly" onfocus="this.blur()"/></td>
            <td></td>
		</tr>
		<tr id="tr_toOrderId" style="display: none;">
			<th width="100" align="right">订单号：</th>
		    <td><input type="text" name="entity.toOrderId" id="toOrderId" size="20"></td>
		</tr>
		<tr>
		...
		<tr>
		    ...
		    <td><input name="entity.appointedTime" type="text" id="appointedTime" onclick="WdatePicker()" readOnly="readonly" style="width: 150px;"></td>
		</tr>
		<tr>
		    ...
		    <td>
		    	<textarea name="entity.descript" id="descript" cols="45" rows="4" 
            		...
            		onblur="javascript:if($(this).val()==''){$(this).val('请填写赔付理据！');this.style.color='#B0B0B0';}"></textarea>
            </td>
		</tr>
		<tr>
...
$(document).ready(function(){
	$("select[name$='big']").each(function(i, n){
		...
		if(giftInfo.length != 0 && $(n).parent().parent().attr("id") != "x"){
			//大类生成下拉框
			$(n).empty();
