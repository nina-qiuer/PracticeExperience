...
function openAppeal(agencyId, agencyName, complaintId) {
	if (confirm("确认开通供应商[" + agencyName + "]投诉单[" + complaintId + "]的异常申诉权限吗？")) {
		var param = {"agencyId":agencyId,"complaintId":complaintId};
		$.ajax({
		type: "POST",
		async:false,
		url: "agency_confirm-openAppeal",
		data: param,
...
<div class="top_crumbs">您当前所在的位置：<a href="#">投诉质检管理</a>>><span class="top_crumbs_txt">供应商赔付确认信息列表</span></div>

<form name="form" id="search_form" method="post" enctype="multipart/form-data" action="agency_confirm" onsubmit="return beforeFormSubmit()">
...
<div class="pici_search pd5 mb10">
  ...
  <label class="mr10">处理状态：
    <select name="page.confirmState">
    	...
    	<option value="-1" <c:if test="${page.confirmState == -1}">selected</c:if>>未对接NB</option>
    	<option value="0" <c:if test="${page.confirmState == 0}">selected</c:if>>未确认</option>
		...
		<option value="4" <c:if test="${page.confirmState == 4}">selected</c:if>>申诉审批中（对接NB）</option>
		<option value="6" <c:if test="${page.confirmState == 6}">selected</c:if>>申诉审批未通过(对接NB)</option>
		<option value="8" <c:if test="${page.confirmState == 8}">selected</c:if>>申诉审批成功（对接NB）</option>
		<option value="5" <c:if test="${page.confirmState == 5}">selected</c:if>>申诉审批中（未对接NB）</option>
		<option value="7" <c:if test="${page.confirmState == 7}">selected</c:if>>申诉审批未通过（未对接NB）</option>
		<option value="9" <c:if test="${page.confirmState == 9}">selected</c:if>>申诉审批成功（未对接NB）</option>		
    </select>
  </label>
...
<table width="100%" class="listtable mb10">
	...
	<tr>
		...
		<td>
		    ...
		    <td align="center">
		    	<c:choose>
					<c:when test="${v.confirmTime == '0000-00-00 00:00:00'}"></c:when>
					<c:otherwise>
						<c:out value="${v.confirmTime}" />
					</c:otherwise>
				</c:choose>
		    </td>
		<td align="center">
		...
		<td align="center"><input title="赔偿通知单"
		    ...
		    <td align="center">
		    	<c:if test="${canOpenAppeal == true}">
		    		<c:if test="${(v.confirmState == '已确认' || v.confirmState == '到期默认') && v.appealInfo == null}">
		    			<input type="button" id="openBtn_${v.agencyId}_${v.complaintId}" title="开通异常申诉权限" class="pd3 mr10" style="cursor: pointer;" value="开通异常申诉" onclick="openAppeal('${v.agencyId}', '${v.agencyName}', '${v.complaintId}')">
		    		</c:if>
		    	</c:if>
		    	<c:if test="${v.confirmState != '未确认' && v.confirmState != '申诉审批中（未对接NB）'&& v.confirmState != '申诉审批中（对接NB）'}">
					<input title="发起OA申请" class="pd3 mr10" style="cursor: pointer;" type="button" value="发起OA申请"
