...
function doSubmit(flag) {
	...
	if (flag < 2) {
		...
		$("#share_solution_form").attr("action", "share_solution-adjustShare");
		$("#share_solution_form").submit();
	}
}
...
<div class="common-box">
<form name="form" id="share_solution_form" method="post" enctype="multipart/form-data" action="share_solution-updateShare">
	<input type="hidden" name="entity.id" value="${entity.id}">
	<input type="hidden" name="entity.complaintId" value="${entity.complaintId }" id="complaintId">
	<input type="hidden" name="entity.orderId" value="${entity.orderId }">
	<input type="hidden" name="entity.total" id="total" value="${entity.total }">
	<input type="hidden" name="entity.supplierTotal" id="supplierTotal" value="${entity.supplierTotal }">
	<input type="hidden" name="entity.qualityToolTotal" id="qualityToolTotal" value="${entity.qualityToolTotal }">
	<input type="hidden" name="entity.employeeTotal" id="employeeTotal" value="${entity.employeeTotal }">
	<input type="hidden" name="entity.saveOrSubmit" id="saveOrSubmit">
	<input type="hidden" name="adjustFlag" id="adjustFlag" value="${adjustFlag }">
	<table class="datatable" width="100%">
		...
		<tr>
		    <th></th>
		    <td id="agencyTable">
		    <c:forEach items="${entity.supportShareList }" var="supportShare" varStatus="st">
		    <table>
    			<tr>
    				<td colspan="2">
						<input name="entity.supportShareList[${st.index}].name" id="agency_name" type="text"  size="14" onchange="checkSupplier(this)" value="${supportShare.name }">　
				        <input name="entity.supportShareList[${st.index}].code" id="agency_code" type="text" size="3"  class="a_code"  readonly value="${supportShare.code }">　　
				        <input name="entity.supportShareList[${st.index}].nbFlag" value="${supportShare.nbFlag }" type="hidden">
				        <c:if test="${1 == supportShare.nbFlag}">
			                <span id="nbFlagText" style="color: green;">[NB供应商赔付确认]</span>
			            </c:if>
				        <input name="entity.supportShareList[${st.index}].foreignCurrencyType" type="hidden" value="${supportShare.foreignCurrencyType }">
				        <input name="entity.supportShareList[${st.index}].foreignCurrencyName" type="hidden" value="${supportShare.foreignCurrencyName }">
			        </td>
    			...
    			<tr>
					...
					<th>承担金额</th>
					<td>
						<input type="button" name="entity.supportShareList[${st.index}].button_info" value="添加赔付" flag="${st.index}" onclick="addRow_supplier(this)"/>
					</td>
				</tr>
				<c:forEach items="${supportShare.agencyPayoutList }" var="payout" varStatus="st2">
					<tr>
					    <td><textarea id="complaintInfo" rows="1" cols="20" name="entity.supportShareList[${st.index}].agencyPayoutList[${st2.index}].complaintInfo" style="width: 173px; height: 21px;">${payout.complaintInfo }</textarea></td>
					    <td class="payoutCl"><textarea id="payoutBase" rows="1" cols="20" name="entity.supportShareList[${st.index}].agencyPayoutList[${st2.index}].payoutBase" style="width: 173px; height: 21px;">${payout.payoutBase }</textarea></td>
					    <td>
					    	<input type="text" name="entity.supportShareList[${st.index}].agencyPayoutList[${st2.index}].payoutNum" onblur="sumSupplierTotal()" value="${payout.payoutNum }" size="10"> 元
					        <c:if test="${payout.foreignCurrencyNumber > 0}"><br>
						        <input class="foreignCurrencyNumber" type="text" name="entity.supportShareList[${st.index}].agencyPayoutList[${st2.index}].foreignCurrencyNumber" value="${payout.foreignCurrencyNumber }" size="10"> 
						        <span class="foreignCurrency">${supportShare.foreignCurrencyName}</span>
						    </c:if>
					    </td>
			 	...
			 	<tr>
			 	    <th>备注</th>
				    <td colspan="2"><textarea id="remark" rows="3" cols="40" name="entity.supportShareList[${st.index}].remark">${supportShare.remark }</textarea></td>
			    </tr>
    		</table>
		    </c:forEach>
		    </td>
		...
		<tr>
			...
			<td><input name="entity.orderGains" type="text" id="orderGains"	size="10" onblur="sumAllTotal()" value="${entity.orderGains }"> 元</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<table border="0" cellpadding="0" cellspacing="0" id="employee_table">
					...
					<tr>
						<td><input name="entity.employeeShareList[xxx].name" type="text" size="10" onblur="checkemploy(this)" value="${empShare.name }"></td>
						<td width="100px"></td>
						<td><input name="entity.employeeShareList[xxx].number" class="employeeList_number" type="text" size="5" onblur="sumEmployeeTotal()" value="${empShare.number }"> 元</td>
						<td align="right"><input type="button" onclick="$(this).parent().parent().remove();sumEmployeeTotal()" value="删除"></td>
					</tr>
					</c:forEach>
				</table>
		...
		<tr>
			...
			<td><input name="entity.special" type="text" id="special" size="10" onblur="sumAllTotal()" value="${entity.special }"> 元</td>
		</tr>
		<tr>
			...
			<td>
				...
				<table border="0" cellpadding="0" cellspacing="0" id="quality_tool_table">
					...
					<c:forEach items="${entity.qualityToolList }" var="quaTool">
					<tr>
						<td>
							<select name="entity.qualityToolList[xxx].toolId" onchange="setToolName(this)">
								<c:forEach items="${toolList}" var="v">
								<c:choose>
								<c:when test="${v.id == quaTool.toolId}">
							...
							<input type="hidden" name="entity.qualityToolList[xxx].toolName" value="${quaTool.toolName }">
						</td>
						<td><input type="text" style="width: 60px;" name="entity.qualityToolList[xxx].total" value="${quaTool.total }" onblur="sumQualityTool()">元</td>
						<td><input type="button" onclick="$(this).parent().parent().remove();sumQualityTool();" value="删除"></td>
					</tr>
					</c:forEach>
				</table>
		...
		<tr>
			...
			<td>
				<input name="entity.refundToIndemnity" type="text" id="refundToIndemnity" size="10" onchange="mutex();" value="${entity.refundToIndemnity }"> 元
...
<!-- 	<input title="赔付理据"  alt="payout_base?name=entity.supportShareList[-1].agencyPayoutList[-1].payoutBase&placeValuesBeforeTB_=savedValues&TB_iframe=true&height=420&width=800&modal=false"
		class="thickbox" name="input" type="button" value="选择" /> -->
...
function addTable_agency() {
	...
	var size = ${entity.supportShareList.size()}+0;
	var index = INCREASE_INDEX + size;
