...
<script type="text/javascript">
	var modelName='新员工薪资调整';
	var mainUrl = 'newEmployeesSalaryAdjustment';
</script>

</head>
<body>
<div class="top_crumbs">您当前所在的位置：投诉质检管理>><span class="top_crumbs_txt">新员工薪资调整</span></div>
<div  class="main">
<form name="form" id="search_form" method="post" enctype="multipart/form-data" action="newEmployeesSalaryAdjustment">
...
<table width="100%" class="listtable mb10">
    ...
    <c:forEach items="${dataList}" var="v">
        <tr>
            ...
            <td>${v.category}</td>
            <td>${v.customerManager}</td>
            <td>${v.workNum}</td>
            <td>${v.businessUnitName}</td>
            <td>${v.departmentName}</td>
            <td>${v.groupName}</td>
            <td>${v.customer}</td>
            <td>
            		<c:if test="${v.isFilToRprtLstMnth == 0 }">否</c:if>
            		<c:if test="${v.isFilToRprtLstMnth == 1 }">是</c:if>
            </td>
			<td>
