	...
	<tbody>
		...
		<tr align="center">
			...
			<c:if test="${v.addPerson == user.realName}">
				<a href="sectionCommissionFiling-delAttach?attachId=${v.id}&id=${id }" onclick="return confirm('本操作不可恢复,确认删除?')">删除</a> 
			</c:if>
			</td>
		</tr>
		</c:forEach>
...
<form id="upload" action="sectionCommissionFiling-upload" enctype="multipart/form-data" method="post">
<input type="hidden" name="id" value="${id }"/>
<table id="file_table_add" class="listtable">
<tr id="tr_note_file" style="display: none">
	<td><input type="file" name="attachFile" onchange="getFileSize(this);"/></td>
	<td>
