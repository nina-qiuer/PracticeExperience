	...
	<tbody>
		...
		<tr align="center">
			...
			<c:if test="${v.addPerson == user.realName}">
				<a href="commissionSummary-delAttach?attachId=${v.id}&id=${id }" onclick="return confirm('本操作不可恢复,确认删除?')">删除</a> 
			</c:if>
			</td>
		</tr>
		</c:forEach>
...
<form id="upload" action="commissionSummary-upload" enctype="multipart/form-data" method="post">
